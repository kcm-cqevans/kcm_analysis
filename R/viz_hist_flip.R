#' Generate a horizontal histogram with customizable appearance
#'
#' This function generates a horizontal histogram plot using ggplot2 with customizable appearance settings.
#'
...
#' @return A ggplot object representing the horizontal histogram.
#' @export
viz_hist_flip <- function(data, outcome_var = "prop", label_var = "proplabel",
                          cat_var = "cat",
                          ymin = 0,
                          ymax = 100,
                          lang = "en",
  ...
  ggplot(data, aes_string(x = factor(data[[cat_var]], levels = data[[cat_var]]), y = data[[outcome_var]])) +
    ...
    geom_text(aes_string(label = data[[label_var]]), vjust = 0.5, hjust = -0.1, size = 5.5, fontface = "bold", color = color_scheme) +
